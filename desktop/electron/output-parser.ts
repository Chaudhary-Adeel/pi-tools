...
export class PiOutputParser {
  ...
  parseOutput(rawLine: string): ParsedEvent[] {
    let withTags = ''
    // Split by ANSI escapes to intercept grey/dim colors
    const parts = rawLine.split(/(\x1B\[[0-9;]*[A-Za-z])/)
    for (const part of parts) {
      if (part.startsWith('\x1B[')) {
        // Check for grey/dim (90m = gray, 2m = dim, 38;5;8m = gray)
        if (part === '\x1B[90m' || part === '\x1B[2m' || part === '\x1B[38;5;8m') {
          if (!this.inThink) {
            withTags += '<think>'
            this.inThink = true
          }
        } 
        // Check for color resets
        else if (part === '\x1B[39m' || part === '\x1B[0m' || part === '\x1B[22m') {
          if (this.inThink) {
            withTags += '</think>'
            this.inThink = false
          }
    ...
    // Fallback: unrecognised line that looks like a Pi output marker.
    if (process.env.NODE_ENV === 'development' && /^(→|✓|✗|─|━|\[|\{|📦|🧠|🔧|⚡)/.test(line)) {
      console.warn('[output-parser] Unrecognised Pi output line:', line)
    }
